...
Dernière mise à jour : 2026-09-20 (L12b). **Document de référence** (voir `docs/methode-de-travail.md`,
...
| **EV-10** *Supprimer* (dans l'éditeur d'un événement existant) | — | *(non proposé : le tiroir ouvre l'éditeur sans suppression)* | `parcours_test.dart` (étape 8 : le bouton est absent) |
...
| Geste | Variantes | Effet observable | Test |
|---|---|---|---|
| **TI-1** Créer un tirage (bouton « + ») | — | tiroir *Créer un tirage* : nom « Tirage N°n », 1 gagnant, min 1 session, **dernière session requise** | `draw_service_test.dart` (`createDraw`) ; `parcours_test.dart` (étape 7) |
| **TI-2** Ouvrir un tirage (clic sur sa ligne) | préparé → *Modifier un tirage* ; effectué → *Consulter un tirage*, champs grisés, sélecteurs ouvrables mais figés | les choix enregistrés sont retrouvés (groupes, sessions, min / max, gagnants) | `parcours_test.dart` (étapes 7, 9) |
...
| **TI-9** *Copier* (icône dans la liste, ou bouton d'un tirage consulté) | source préparée / effectuée | tiroir *Créer un tirage* sur une copie **non enregistrée** : mêmes réglages, nom « Tirage N°n », et le groupe des gagnants de la source parmi les groupes exclus (si effectuée) | `draw_service_test.dart` (`createDrawFromDraw`) ; `parcours_test.dart` (étape 7) |
...
parcours joue les 74, sauf trois dialogues du système et une animation, en recette.
